...
# 4. Create the database schema (managed by Alembic)
...
The Qdrant collection is created automatically on startup (idempotent). The
Postgres schema is managed by Alembic — see below.

## Database migrations (Alembic)

The schema is owned by Alembic, wired to the app config (`alembic/env.py` reads
the DB URL from `.env`) and the ORM models (`app.db.Base`). Common commands:
...
Typical workflow: change the models in `app/models.py`, run
`alembic revision --autogenerate -m "..."`, review the generated file in
`alembic/versions/`, then `alembic upgrade head`. Point Alembic at a different
database with the `ALEMBIC_DATABASE_URL` env var.

> Note on native enums: PostgreSQL enum types are not dropped automatically
> when a table is dropped, so autogenerated `downgrade()` functions may need an
> explicit `sa.Enum(name=...).drop(op.get_bind())` (as in the initial revision).
...
and embedding models inside the cluster.
